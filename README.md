...
<p align="center">
  ...
  <a href="./Docs/TataIQ_@Forage_GenAI_Certificate.pdf">view the completion certificate</a>
</p>
...
This project simulates the role of a data analyst on the Financial Services team at **Tata iQ**, tasked with building a system to predict customer **delinquency risk** and recommend **ethical, automated collections strategies** informed by explainable AI.
...
Lenders and subscription-based financial service providers routinely lose revenue and increase operational cost because they cannot identify at-risk accounts early enough. Three problems recur across the industry:
...
- **Opaque, potentially unfair decisions** — collections actions driven by a "black box" model risk breaching fair-lending expectations if the model leans on proxies for age, employment status, or location.
...
**Data quality note surfaced during EDA:** `Employment_Status` contains inconsistent labeling for the same categories (e.g. `EMP`, `Employed`, `employed`) that has not yet been standardized in the pipeline — see Section 19.
...
as predictors. The regression is trained only on employed customers with complete data, then applied to the employed customers with missing income — a more defensible approach than segment-agnostic imputation.
...
4. **Class imbalance handling:** `SMOTE` is applied **only to the training set** (after preprocessing), so the model learns from a balanced set of examples without leaking synthetic samples into evaluation.
5. **Model:** `RandomForestClassifier(n_estimators=200, random_state=42)`.
6. **Risk flagging:** predicted probabilities are compared against a configurable threshold (`flagging_threshold`) to produce a `flagged_as_risk_sensitive` column for the collections team, in addition to the raw predicted probabilities.
...
**Honest read of these numbers:** overall accuracy looks strong, but that is driven almost entirely by the majority (non-delinquent) class — the model currently identifies only a small fraction of the truly delinquent test customers (1 out of 20). This is the key finding that justifies continued investment in model improvement and human review.

A second artifact in the same CSV, `flagged_as_risk_sensitive`, applies a **lower, recall-oriented threshold (~0.2 rather than 0.5)** and flags 79 of 125 accounts as risk-sensitive — closer to the business goal of not missing at-risk customers.

## 12. Explainability (SHAP)

The current codebase implements **feature importance from the trained Random Forest** (`model.feature_importances_`, visualized in `feature_importances.png`) as a first-pass, global view of which features drive the model's predictions.

**SHAP-based explainability is part of the project's design and business proposal** (justified during the Forage simulation as the mechanism for producing per-customer, auditable explanations — e.g., "this customer is flagged because of high Missed_Payments + low Credit_Score").

## 13. Fairness Assessment

Similarly, a **Fairlearn-based fairness assessment** — checking whether false-negative/false-positive rates differ meaningfully across `Employment_Status`, `Age` bands, or `Location` — was proposed as a gating step before model deployment, but not yet implemented in the current scripts.
...
- A **configurable risk threshold** that lets the business trade off between "review more accounts" (higher recall, more manual workload) and "review fewer accounts" (higher precision, more risk of missed delinquencies).
...
- The current recall gap on the delinquent class (Section 10) is itself a useful business finding: it shows this first iteration is **not yet ready to replace manual review**, and quantifies exactly how much model improvement is needed.
...
3. **Route by tier**: low-risk accounts receive automated, low-touch reminders; medium-risk accounts trigger an outreach workflow; high-risk accounts are escalated to a human collections agent with a full summary of the account history and model reasoning.
...
> **Note:** the scripts currently reference local Windows file paths (e.g. `C:\Users\prave\...`). Update these to relative paths before running on another machine — see [Installation](#17-installation).
...
Before running the scripts, update the hard-coded file paths in `EDA_Databse.py`, `Imputation.py`, and `RandomForestClassifier_model.py` to point to your local copy of `Fully_Cleaned_Delenquency_Prediction_Dataset.xlsx`.
...
- **Close the reproducibility gap** between the committed `predicted_delinquent_accounts_with_flags.csv` and the current script's hard-coded 0.5 threshold (Section 11), and re-generate the export from scratch to confirm consistency.
- **Improve recall on the delinquent class** — investigate whether SMOTE parameters, class weighting, alternative models (e.g. gradient boosting), or richer features from `Month_1`–`Month_6` payment behavior can boost delinquent-class recall toward 0.70+.
- **Implement SHAP explainability** (Section 12) on top of the trained pipeline for per-customer, auditable explanations.
- **Implement the Fairlearn fairness assessment** (Section 13) across `Employment_Status`, `Age`, and `Location`.
- **Standardize categorical labels** — `Employment_Status` currently contains inconsistent casing/variants of the same category (e.g. `EMP`, `Employed`, `employed`) that should be normalized before modeling.
